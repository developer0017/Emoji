...
@property (nonatomic, strong) UIView *btmBar;
...
//    self.textView.layer.borderWidth = 1.0f;
    ...
    self.barHeight = 2.0f;
//    self.btmBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.lineHeight + kViewPadding - self.barHeight, self.frame.size.width, self.barHeight)];
    self.btmBar = [[UIView alloc] initWithFrame:CGRectMake(0, self.textView.frame.origin.y + self.textView.frame.size.height + kViewPadding - self.barHeight, self.frame.size.width, self.barHeight)];
    [self.btmBar setBackgroundColor:[UIColor whiteColor]];
    [self addSubview:self.btmBar];
...
- (void)layoutView {
    ...
    CGRect barFrame = self.btmBar.frame;
    barFrame.origin.y = newHeight - self.barHeight;
    self.btmBar.frame = barFrame;
...
{
    self.textView.hidden = NO;
    [self.delegate contactPickerShouldHideDoneButton:NO];
...
{
    [self.delegate contactPickerShouldHideDoneButton:NO];
    if (tView == self.textView) {
        NSLog(@"Contact Text Selected");
    } else {
...
{
    [self.delegate contactPickerShouldHideDoneButton:YES];
